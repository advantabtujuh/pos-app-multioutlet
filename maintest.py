"""
main.py
Entry point utama M4-COMPACT POS & Inventory System.
Alur: DB init -> Login -> Main Window -> Role Filter -> Dashboard

CATATAN: Label menu pakai plain text kapital semua,
         TANPA emoji, untuk menghindari unicode encoding mismatch.
"""
...
# ==============================================================================
# KONSTANTA: ROLE-BASED MENU PERMISSIONS
# ==============================================================================
MENU_PERMISSIONS = {
    "ADMIN": [
        "DASHBOARD",
    ...
    "GUDANG": [
        ...
        "KATEGORI", "BARANG", "SUPPLIER",
        "MUTASI",
        # "PEMBELIAN" ← GUDANG bisa akses tapi tidak bisa POSTING
...
class CollapsibleSection(QWidget):
    def __init__(self, title: str, parent=None):
        ...
        self._inner.setContentsMargins(5, 5, 5, 5)
        root.addWidget(self._container)

        self._child_buttons: list = []
    ...
    def get_child_buttons(self):
        return self._child_buttons

    def allowed_item_count(self, allowed_buttons_set) -> int:
        count = 0
        for btn in self._child_buttons:
            if btn in allowed_buttons_set:
                count += 1
        return count
...
class MainWindowPOS(QMainWindow):
    ...
    # Signal untuk notify app bahwa user mau logout (bukan exit)
    ...
    def __init__(self, user_data: dict):
        ...
        raw_role = user_data.get("role", "KASIR")
        if raw_role is None:
            raw_role = "KASIR"
        ...
        if self._role not in MENU_PERMISSIONS:
            print(f"[WARNING] Role '{self._role}' tidak dikenali! Fallback ke KASIR.")
            self._role = "KASIR"
            self.current_user["role"] = "KASIR"
        
        self.current_user["role"] = self._role

        self.setWindowTitle(
            f"M4-COMPACT POS  |  "
            f"{user_data['nama_lengkap']}  [{self._role}]"
        )
        self.showMaximized()
        ...
        self.content.setStyleSheet(
            "#MainContent { background-color: #f4f6f9;"
        ...
        print(f"[INFO] User: {self.current_user['nama_lengkap']}, Role: {self._role}")
    ...
    def _build_sidebar(self) -> QFrame:
        ...
        scroll.setStyleSheet("""
            ...
            QScrollBar::handle:vertical {
                background: #374850; border-radius: 2px;
            }
        """)

        menu_root = QWidget()
        menu_root.setStyleSheet("background: transparent;")
        menu_layout = QVBoxLayout(menu_root)
        menu_layout.setSpacing(2)
        menu_layout.setContentsMargins(10, 15, 10, 15)

        for sec_title, items in self.SECTIONS_DEF:
            sec = CollapsibleSection(sec_title)
            sec.set_expanded(True)
            for key, display, *_ in items:
    ...
    def _build_sidebar_header(self) -> QWidget:
        ...
        lbl_toko = QLabel("NAMA TOKO")
        lbl_toko.setFont(get_font(14, bold=True))
        lbl_toko.setStyleSheet("color: #f1f5f9; letter-spacing: 2px;")
        layout.addWidget(lbl_toko)

        line = QFrame()
        line.setFrameShape(QFrame.HLine)
        line.setFixedHeight(1)
        line.setStyleSheet("background-color: #374850;")
        layout.addWidget(line)

        role_colors = {
            "ADMIN": "#e74c3c", "MANAGER": "#9b59b6",
            "GUDANG": "#f39c12", "KASIR":   "#3498db",
        }
        lbl_user = QLabel(
            f"  {self.current_user['nama_lengkap']}  |  {self._role}"
        )
        lbl_user.setFont(get_font(9, bold=True))
        lbl_user.setStyleSheet(
            f"color: {role_colors.get(self._role, '#3b82f6')};"
        )
        lbl_user.setWordWrap(True)
        layout.addWidget(lbl_user)
        return header

    def _make_menu_btn(self, key: str, display: str, callback) -> QPushButton:
        btn = QPushButton(display)
        btn.setProperty("menu_key", key)
        btn.setFont(get_font(9))
    ...
    def _apply_role_filter(self):
        ...
        for btn in self._menu_buttons:
            ...
            is_allowed = key in allowed
            btn.setVisible(is_allowed)
            if is_allowed:
                self._allowed_buttons.add(btn)

        for sec in self._sections:
            allowed_in_sec = sec.allowed_item_count(self._allowed_buttons)
            sec.setVisible(allowed_in_sec > 0)
    ...
    def _build_dashboard(self):
        ...
        lbl_title.setFont(get_font(22, bold=True))
        lbl_title.setStyleSheet("color: #1e293b;")
        layout.addWidget(lbl_title)

        lbl_sub = QLabel(
            f"Selamat datang, {self.current_user['nama_lengkap']}  —  "
            f"{datetime.now().strftime('%A, %d %B %Y')}"
        )
        lbl_sub.setFont(get_font(10))
        ...
        for c in [self.card_barang, self.card_stok,
                  self.card_penjualan, self.card_member]:
        ...
        for c in [self.card_hutang_member,
                  self.card_hutang_supplier,
                  self.card_hutang_customer]:
            c.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
            row2.addWidget(c)
    ...
    def _ph_mutasi(self):        self._open_placeholder("Mutasi Barang")
    ...
    def _ph_settings(self):      self._open_placeholder("Settings Aplikasi")
    ...
    def _open_widget(self, widget_class):
        for i in range(self.content.count()):
            ...
            if type(w) is widget_class:
                ...
                if hasattr(w, "muat_data_dari_db"):
                    w.muat_data_dari_db()
                return
        w = widget_class(self)
        w.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.content.addWidget(w)
        self.content.setCurrentWidget(w)
    ...
    def _open_placeholder(self, title: str):
        ...
        page = QWidget()
        page.setProperty("ph_title", title)
        page.setStyleSheet("background-color: #f4f6f9;")
        layout = QVBoxLayout(page)
        ...
        ico = QLabel("🚧")
        ico.setFont(get_font(52))
        ico.setAlignment(Qt.AlignCenter)

        lbl = QLabel(title)
        lbl.setFont(get_font(16, bold=True))
        lbl.setStyleSheet("color: #1e293b;")
        lbl.setAlignment(Qt.AlignCenter)
        lbl.setWordWrap(True)

        sub = QLabel("Modul dalam pengembangan")
    ...
    # ==========================================================================
    # LOGOUT - Emit signal instead of closing app
    # ==========================================================================
    def _do_logout(self):
...
def run_app():
    """Main application loop dengan support logout -> relogin."""
