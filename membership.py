...
def init_member_database():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS master_member (id BLOB PRIMARY KEY)")
    cursor.execute("PRAGMA table_info(master_member)")
    kolom_master = [info[1] for info in cursor.fetchall()]

    if "kode_member" not in kolom_master:
        cursor.execute("ALTER TABLE master_member ADD COLUMN kode_member TEXT NOT NULL DEFAULT ''")
    if "nama_member" not in kolom_master:
        cursor.execute("ALTER TABLE master_member ADD COLUMN nama_member TEXT NOT NULL DEFAULT ''")
    if "alamat_member" not in kolom_master:
        cursor.execute("ALTER TABLE master_member ADD COLUMN alamat_member TEXT NOT NULL DEFAULT ''")
    if "telepon_member" not in kolom_master:
        cursor.execute("ALTER TABLE master_member ADD COLUMN telepon_member TEXT NOT NULL DEFAULT ''")
    if "ttl_hutang" not in kolom_master:
        cursor.execute("ALTER TABLE master_member ADD COLUMN ttl_hutang REAL NOT NULL DEFAULT 0.0")
    if "status_hutang" not in kolom_master:
        cursor.execute("ALTER TABLE master_member ADD COLUMN status_hutang TEXT NOT NULL DEFAULT 'LUNAS'")

    cursor.execute("""
        CREATE TABLE IF NOT EXISTS dethut_member (
            id BLOB PRIMARY KEY, member_id BLOB NOT NULL, tanggal TEXT NOT NULL, jenis_transaksi TEXT NOT NULL,
            debet REAL NOT NULL DEFAULT 0.0, kredit REAL NOT NULL DEFAULT 0.0, total_hutang REAL NOT NULL DEFAULT 0.0,
            FOREIGN KEY(member_id) REFERENCES master_member(id)
        )
    """)
    conn.commit()
    conn.close()

class FormMemberDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("✏️ Edit Data Member" if is_edit else "➕ Tambah Member Baru")
        self.setModal(True)
        self.resize(420, 380)
        ...
        self.input_kode.setPlaceholderText("Kode unik member")
        self.input_kode.setStyleSheet(self._input_style())

        self.input_nama = QLineEdit(self)
        self.input_nama.setPlaceholderText("Nama lengkap member")
        ...
        self.input_telp.setPlaceholderText("Nomor telepon")
        self.input_telp.setStyleSheet(self._input_style())

        self.input_hutang_awal = QLineEdit(self)
        self.input_hutang_awal.setPlaceholderText("0")
        self.input_hutang_awal.setStyleSheet(self._input_style())

        form_layout.addRow("🔑 Kode Member:", self.input_kode)
        form_layout.addRow("👤 Nama Member:", self.input_nama)
        ...
        if self.data_edit:
            self.input_kode.setText(self.data_edit['kode_member'])
            self.input_kode.setReadOnly(True) 
            self.input_kode.setStyleSheet(self._input_style() + "background-color: #f1f2f6;")
            self.input_nama.setText(self.data_edit['nama_member'])
            self.input_alamat.setText(self.data_edit['alamat_member'])
            self.input_telp.setText(self.data_edit['telepon_member'])
    ...
    def proses_simpan(self):
        ...
        if not kode or not nama:
            QMessageBox.warning(self, "Peringatan", "⚠️ Kode dan Nama Member wajib diisi!")
            return
        ...
        if self.data_edit:
            ...
            cursor.execute("UPDATE master_member SET nama_member=?, alamat_member=?, telepon_member=? WHERE id=?", (nama, alamat, telp, id_bin))
            self.saved_id = id_bin
        else:
            cursor.execute("SELECT id FROM master_member WHERE kode_member = ?", (kode,))
            if cursor.fetchone():
                QMessageBox.warning(self, "Peringatan", "⚠️ Kode Member sudah terpakai!")
                conn.close()
                return
            hutang_str = self.input_hutang_awal.text().strip() or "0"
            try:
                hutang_awal = float(hutang_str)
            except ValueError:
            ...
            cursor.execute("INSERT INTO master_member VALUES (?,?,?,?,?,?,?)", (id_bytes, kode, nama, alamat, telp, hutang_awal, status_awal))
            if hutang_awal > 0:
                cursor.execute("INSERT INTO dethut_member VALUES (?,?,?,?,?,?,?)", (uuid.uuid4().bytes, id_bytes, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Hutang Awal", hutang_awal, 0.0, hutang_awal))

        conn.commit()
        conn.close()
        self.accept()

class DetailHutangMemberDialog(QDialog):
    def __init__(self, parent, member_id, nama_mbr, kode_mbr):
        super().__init__(parent)
        self.member_id, self.nama_mbr, self.kode_mbr = member_id, nama_mbr, kode_mbr
        self.init_ui()
        self.filter_data()

    def init_ui(self):
        self.setWindowTitle(f"📜 Riwayat Kartu Hutang Member - {self.nama_mbr}")
        self.resize(750, 450)
    ...
    def filter_data(self):
        ...
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("SELECT tanggal, jenis_transaksi, debet, kredit, total_hutang FROM dethut_member WHERE member_id=? AND tanggal BETWEEN ? AND ? ORDER BY tanggal ASC", (self.member_id, tgl_m, tgl_s))
    ...
    def proses_pdf_pyqt(self):
        ...
        <html><head><style>
            ...
            <h2>KARTU RIWAYAT PIUTANG/HUTANG MEMBER</h2>
            <table class="meta-table">
                <tr><td><b>🔑 Kode Member :</b> {self.kode_mbr}</td><td style="text-align:right;"><b>📅 Periode :</b> {self.date_mulai.date().toString('dd/MM/yyyy')} s/d {self.date_selesai.date().toString('dd/MM/yyyy')}</td></tr>
                <tr><td><b>👤 Nama Member :</b> {self.nama_mbr}</td><td style="text-align:right;"><b>🖨️ Tanggal Cetak :</b> {datetime.now().strftime('%d/%m/%Y %H:%M')}</td></tr>
            </table>
...
class MemberWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(["ID Hidden", "🔑 Kode", "👤 Nama Member", "📍 Alamat", "📞 No. Telepon", "💰 Total Hutang", "📌 Status"])
    ...
    def muat_data_dari_db(self, target_highlight_id=None):
        ...
        cursor = conn.cursor()
        if keyword:
            cursor.execute("SELECT id, kode_member, nama_member, alamat_member, telepon_member, ttl_hutang, status_hutang FROM master_member WHERE nama_member LIKE ? OR kode_member LIKE ?", (f"%{keyword}%", f"%{keyword}%"))
        else:
            cursor.execute("SELECT id, kode_member, nama_member, alamat_member, telepon_member, ttl_hutang, status_hutang FROM master_member")
    ...
    def aksi_edit(self):
        ...
        data_edit = {'id': id_bin, 'kode_member': self.table.item(b, 1).text(), 'nama_member': self.table.item(b, 2).text(), 'alamat_member': self.table.item(b, 3).text(), 'telepon_member': self.table.item(b, 4).text()}
        dialog = FormMemberDialog(self, data_edit=data_edit)
        if dialog.exec_() == QDialog.Accepted: self.muat_data_dari_db(target_highlight_id=dialog.saved_id)
    ...
    def aksi_hapus(self):
        ...
        if self.table.item(b, 6).text() == "BELUM LUNAS":
            QMessageBox.critical(self, "Aksi Ditolak!", "⚠️ Member ini masih berhutang bon! Selesaikan pembayaran nota dulu.")
            return
        id_bin = self.table.item(b, 0).data(Qt.UserRole)
        if QMessageBox.question(self, "Konfirmasi", f"❓ Hapus master data member '{self.table.item(b,2).text()}'?", QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute("DELETE FROM dethut_member WHERE member_id = ?", (id_bin,))
            cursor.execute("DELETE FROM master_member WHERE id = ?", (id_bin,))
            conn.commit()
            conn.close()
            self.muat_data_dari_db()
    ...
    def aksi_detail(self):
        b = self.table.currentRow()
        if b == -1: return
        dialog = DetailHutangMemberDialog(self, self.table.item(b, 0).data(Qt.UserRole), self.table.item(b, 2).text(), self.table.item(b, 1).text())
        dialog.exec_()
